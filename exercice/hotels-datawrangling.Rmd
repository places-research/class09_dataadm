...
Aquecimento!
Dê uma olhada geral nos dados com a função `skim()`.

**Nota:** Eu já dei a resposta a este exercício.
Você só precisa fazer o `knit` do documento e visualizar a saída.
Uma definição de todas as variáveis é dada na seção [Dicionário de dados] no final, embora você não precise se familiarizar com todas as variáveis para poder trabalhar com estes exercícios.
...
Preencha os espaços em branco para filtragem de reservas de hotel onde o hóspede é **não** dos EUA (código do país `EUA`) e o `lead_time` é menos de 1 dia.
